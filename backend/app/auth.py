...
async def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    try:
        ...
        if not user:
            raise HTTPException(status_code=404, detail="User not found in our database. Please contact an administrator.")

        # If user is found by email, update their ID with the Firebase UID
        if user.id != uid:
            user.id = uid
            db.commit()
            db.refresh(user)
...
require_promoter = require_role(UserRole.PROMOTER)
